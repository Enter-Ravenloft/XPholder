    ...
    switch (view) {
      ...
      case "channels":
        ...
        for (const gameChunk of gameChunks) {
          gameEmbed.push(
            await buildChannelEmbed(
              mergeListOfObjects(gameChunk),
              interaction,
              guildService
            )
          );
...
function buildConfigEmbed(configObj, tiersArr) {
  ...
  tiersArr.forEach((tier, index) => {
    tierFields.push({
      inline: true,
      name: `Character Tier ${index + 1}`,
      value: `<@&${tier["role_id"]}>
              **XP Bonus:** ${tier["xp_bonus"]}
              **Level Range:** ${tier["minimum_level"]}-${tier["maximum_level"]}`,
    });
  });
    ...
    .setFields(
      {
        inline: true,
        name: "Approve Level",
        value: `${configObj.approveLevel}`,
      },
      {
        inline: true,
        name: "Character 1",
        value: `<@&${configObj.character1RoleId}>`,
      },
      {
        inline: true,
        name: "Character 2",
        value: `<@&${configObj.character2RoleId}>`,
      },
      {
        inline: true,
        name: "Character 3",
        value: `<@&${configObj.character3RoleId}>`,
      },
      {
        inline: true,
        name: "Character 4",
        value: `<@&${configObj.character4RoleId}>`,
      },
      {
        inline: true,
        name: "Character 5",
      ...
      // },
      {
        ...
        value: `<#${configObj.levelUpChannelId}>`,
      },
      {
        inline: true,
        name: "Mod Role",
        value: `<@&${configObj.moderationRoleId}>`,
      },
      ...
      {
        inline: true,
        name: "Allow Player Manage Xp",
        value: `${configObj.allowPlayerManageXp}`,
      },
      {
        inline: true,
        name: "XP Freeze",
        value: `<@&${configObj.xpFreezeRoleId}>`,
      },
      {
        inline: true,
        name: "XP Share",
        value: `<@&${configObj.xpShareRoleId}>`,
      },
      {
        inline: true,
        name: "XP Per-Post Divisor",
        value: `${configObj.xpPerPostDivisor}`,
      },
      {
        inline: true,
        name: "XP Per-Post Formula",
        value: `${configObj.xpPerPostFormula}`,
      },
      ...tierFields
    )
...
function buildLevelEmbed(levelObj) {
  ...
  for (const [level, xpToNextLevel] of Object.entries(levelObj)) {
    levelEmbed.addFields({
      name: `lv ${level}`,
      value: `**Start xp:** ${requiredXp}\n**XP to lv:** ${xpToNextLevel}`,
      inline: true,
    });
...
async function buildTiersEmbed(tiersArray) {
  const tiersEmbed = new EmbedBuilder()
    .setTitle("Tiers")
    .setFooter({ text: `Dont Like What You See? Try : /edit_tiers` })
    .setThumbnail(XPHOLDER_ICON_URL)
    .setColor(XPHOLDER_COLOUR)
    .setURL(DEV_SERVER_URL);
  for (const tier of tiersArray) {
    tiersEmbed.addFields({
      name: `ID : ${tier["role_id"]}`,
      value: `<@&${tier["role_id"]}>
                     **XP Bonus : ** ${tier["xp_bonus"]}
                     **level range :** ${tier["minimum_level"]}-${tier["maximum_level"]}`,
      inline: true,
