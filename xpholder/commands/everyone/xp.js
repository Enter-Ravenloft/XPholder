...
module.exports = {
  ...
  async execute(guildService, interaction) {
    ...
    const activeCharacterNumber = getActiveCharacterNumber(
      guildService.config,
      player._roles
    );
    ...
    if (player.id == interaction.user.id && !public) {
      characterButtons = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
    ...
    await interaction.editReply({
      embeds: [characterEmbeds[embedCharacterIndex]],
  ...
  async handleXpCommandButton(guildService, interaction) {
    ...
    let embedCharacterNumber = parseInt(
      originalEmbed.fields.filter((field) => field.name === "Character No.")[0]
        .value
    );
    ...
    let retire =
      ...
      originalEmbed.description.includes(
        "**WARNING:** Are you sure you want to retire?"
      );
    ...
    try {
      ...
      switch (interaction.customId) {
        ...
        case "xp_set":
          ...
          const newCharacterLevelInfo = getLevelInfo(
            guildService.levels,
            newCharacter["xp"]
          );
          const newCharacterTier = getTier(
            parseInt(newCharacterLevelInfo["level"])
          );
          ...
          for (
            let charIndex = 1;
            charIndex <= guildService.config.characterCount;
            charIndex++
          ) {
            ...
            removeRoles.push(
              await guild.roles.fetch(
                guildService.config[`character${charIndex}RoleId`]
              )
            );
          }
          addRoles.push(
            await guild.roles.fetch(
              guildService.config[
                `character${newCharacter["character_index"]}RoleId`
              ]
            )
          );
          ...
          for (let tierIndex = 1; tierIndex <= 4; tierIndex++) {
            ...
            removeRoles.push(
              await guild.roles.fetch(
                guildService.config[`tier${tierIndex}RoleId`]
              )
            );
          }
          addRoles.push(
            await guild.roles.fetch(
              guildService.config[`tier${newCharacterTier["tier"]}RoleId`]
            )
          );

          const updatedPlayer = await player.roles.remove(
            removeRoles.filter((r) => r)
          );
